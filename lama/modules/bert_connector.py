...
class Bert(Base_Connector):
    ...
    def get_batch_generation(self, sentences_list, logger= None,
                             try_cuda=True, return_features=False):
        ...
        with torch.no_grad():
            ...
            if return_features:
                ...
                features_tensor = torch.cat([features[i, j, :] for i, j in enumerate(masked_indices_list)], dim=0)
        ...
        if return_features:
            return log_probs, token_ids_list, masked_indices_list, features_tensor
        else:
